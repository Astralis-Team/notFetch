...
```bash
npm install @astralis-os/notfetch
```
...
const response = await notfetch.get<User[]>('/users');
```
...
const getUsers: ApiNotFetchRequest<GetUsersParams, any[]> = ({ params, config }) =>
  notfetch.get('/users', {
    ...config,
...
The notfetch library provides flexible response parsing options to handle different types of responses. You can specify how to parse the response body using predefined modes or custom functions. If no parse mode is specified, the response will be automatically parsed based on the `Content-Type` header:

```typescript
const response = await notfetch.get('/users', { parse: 'json' });
```
...
```typescript
const response = await notfetch.get('/users', { parse: (data) => data.json() });
```
...
```typescript
const response = await notfetch.get('/binary-file', { parse: 'raw' });
```
...
```typescript
const response = await notfetch.get('/users', { context: 'data' });
